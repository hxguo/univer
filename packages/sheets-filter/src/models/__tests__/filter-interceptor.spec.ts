...
import { Direction, ICommandService, IUniverInstanceService, Plugin, PluginType, RANGE_TYPE, Univer } from '@univerjs/core';
...
describe('Test "Filter Interceptor"', () => {
    ...
    beforeEach(() => {
        ...
        [InsertColMutation, RemoveColMutation, InsertColCommand, InsertRowMutation, RemoveRowMutation, InsertRowCommand, RemoveColCommand, RemoveRowCommand, MoveColsMutation, MoveColsCommand, MoveRowsCommand, MoveRowsMutation, SetSelectionsOperation, RemoveSheetCommand, RemoveSheetMutation].forEach((command) => {
            commandService.registerCommand(command);
        });
        getFilter = (unitId = 'workbookId', sheetId = 'worksheetId') => {
            return univerInstanceService.getUniverSheetInstance(unitId)?.getSheetBySheetId(sheetId)?.getSnapshot().autoFilter;
        };
    });
    ...
    describe('Test "remove Command"', () => {
        it('remove col command, in filter range', async () => {
            ...
            expect(sheetsFilterService.getFilterModel('workbookId', 'worksheetId')?.getAllFilterColumns().map((x) => x[0])).toStrictEqual([0]);
        });
        ...
        it('remove row command, in filter range', async () => {
            ...
            expect(sheetsFilterService.getFilterModel('workbookId', 'worksheetId')?.getRange()).toStrictEqual({ startColumn: 1, endColumn: 2, startRow: 1, endRow: 1 });
            expect(sheetsFilterService.getFilterModel('workbookId', 'worksheetId')?.getAllFilterColumns().map((x) => x[0])).toStrictEqual([1]);
        });
...
export function createFilterTestUniver(dependencies?: Dependency[], workbookData?: IWorkbookData
) {
    ...
    univer.createUniverSheet(workbookData || TEST_WORKBOOK_DATA_DEMO());
